...
const server = createServer(async (req, res) => {
  ...
  assert.equal(JSON.parse(body).model, 'fixture-vision')
...
try {
  ...
  assert.deepEqual(requests, ['/v1/models', '/v1/messages', '/v1/models'])
  console.log('Settings Electron smoke passed: locked text, images saved/reloaded, 404 recovery, manual model probe, zh/en; requests=3')
} finally {
