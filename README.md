...
- 差异查看、应用 / 撤销，随时取消（取消映射为 `ABORTED`，不会污染错误统计）。
...
- **真实 Key 下的成功路径还没有验证过。** 三种协议、取消、冲突、鉴权失败都用本地 HTTP 服务覆盖了，但没有用真实凭据跑通过一次真实模型响应；
- **有一次客户端闪退的根因没有复现。** 日志时间线里没有 JS 层异常，所以只补齐了日志证据链（未捕获异常、子进程退出、渲染进程崩溃、生成请求终态），没有声称修好了根因；
- **模型列表的端点路由是防御性加固。** 兼容服务在哪儿提供模型列表并不统一，代码按同源候选路径依次尝试，但哪条路径对我用的服务真正可用，还没有实证。
