...
这个仓库就是这份答卷：不是给 Nexus-Editor 提一个局部 PR，而是把它「Markdown 源文本为唯一真值，编辑器 / 预览 / 索引 / AI 全部从源文本派生」的主张，用 Electron + TypeScript 独立重写了一遍。代码是自己写的，没有二开 Nexus-Editor，仓库里也不包含该项目的任何源码（见 [关于独立开发](#关于独立开发)）。
...
本项目采用 [PolyForm Noncommercial License 1.0.0](LICENSE)：个人学习、研究、实验、业余项目以及慈善、教育、公共研究、公共卫生、环保和政府部门等非营利组织的使用都是允许的，**任何商业用途都不被许可**。

需要商用授权请通过 <https://github.com/youxiandechilun> 联系作者。
...
- **Nexus-Editor**：只参考了用户提供的项目介绍（用于确定功能定位），**没有审查或使用其源码**，仓库中不包含该项目代码，见 [参考项目与使用边界](docs/01_需求/需求规格说明书.md)；
